...
ALLOWED_THEMES = ("neutral", "soft-cat", "strict-terminal", "warm-sunrise", "ocean-calm", "fluffy-cat", "strict-grid")
...
def build_gigabuddy_persona(drive_root: pathlib.Path | str, query: str = "") -> str:
    ...
    if not has_track:
        ...
        scenario_block = (
            ...
            "5. Когда трек определён — ОБЯЗАТЕЛЬНО предложи новичку зафиксировать его (например: «давай зафиксируем твой трек — он появится в левой колонке с галочками»). После согласия новичка трек записывается в персистентное состояние и появляется в трекере слева.\n\n"
            f"{methodology_block}\n"
        )
    else:
        scenario_block = (
