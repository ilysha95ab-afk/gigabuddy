...
- `web/modules/gigabuddy.js` owns the reversible GigaBuddy product-mode demo shell: synthetic onboarding-track state, stage labels (Советчик → Помощник → Партнёр), product-mode body-class application from `/api/settings`, and the main-chat adaptation-track sidecar. It deliberately uses the settings seam (`OUROBOROS_PRODUCT_MODE=gigabuddy`) rather than adding a frozen `/api/state` field.
...
| OUROBOROS_PRODUCT_MODE | "" | Reversible presentation overlay. Empty keeps ordinary Ouroboros; `gigabuddy` enables the GigaBuddy novice-facing onboarding shell (branding, adaptation-track sidecar, hidden developer chrome) through the existing settings seam. Presentation only: it does not replace BIBLE/identity and does not remove emergency stop/owner-admin control paths. |
