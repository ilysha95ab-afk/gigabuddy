# Ouroboros v6.79.1 — Architecture & Reference
...
- `web/modules/gigabuddy.js` owns the reversible GigaBuddy product-mode demo shell: synthetic onboarding-track state, stage labels (Советчик → Помощник → Партнёр), product-mode body-class application from `/api/settings`, a server-verified GigaBuddy return PIN action on that same settings seam, and the main-chat adaptation-track sidecar. It deliberately uses the settings seam (`OUROBOROS_PRODUCT_MODE=gigabuddy`) rather than adding a frozen `/api/state` field.
...
Settings has Providers, Secrets, Models, Behavior, Advanced, and About. It handles provider keys, model routing, review settings, runtime mode, product presentation mode, external skills repo, ClawHub registry URL, MCP servers, source control metadata, local model runtime, extension settings, timeouts, and reset. Product Mode is a normal owner/admin setting: empty keeps ordinary Ouroboros, while `gigabuddy` enables the reversible GigaBuddy onboarding shell after the frontend reloads settings (restart is the safe demo activation path). The active shell exposes a visible owner/admin return form; the submitted four-digit `GIGABUDDY_ADMIN_PIN` is checked only on the server via the existing `/api/settings` action branch, and generic settings saves cannot clear product mode or overwrite the PIN while GigaBuddy is active. It remains local owner/admin recovery, not public authentication, and does not remove emergency stop paths. Hot-reload policy: total budget, timeouts, GitHub metadata, and the frontend product-mode presentation apply immediately after settings refresh; per-task cost threshold, models, API keys, effort, and review settings apply next task; local runtime, worker count, base URLs, provider runtime parameters, and runtime-mode changes require restart. Runtime mode remains owner-controlled: ordinary `/api/settings` drops it, while `/api/owner/runtime-mode` persists the next-boot value without changing the current boot baseline.
...
| OUROBOROS_PRODUCT_MODE | "" | Reversible presentation overlay. Empty keeps ordinary Ouroboros; `gigabuddy` enables the GigaBuddy novice-facing onboarding shell (branding, adaptation-track sidecar, hidden developer chrome) through the existing settings seam. While active, ordinary settings saves cannot clear the mode; the visible return form uses server-side `GIGABUDDY_ADMIN_PIN` verification. Presentation only: it does not replace BIBLE/identity and does not remove emergency stop/owner-admin control paths. |
