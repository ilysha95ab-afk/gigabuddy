...
- ярлык **Ouroboros** появится в **меню приложений** (`~/.local/share/applications/`);
- и на **рабочем столе** (`~/Desktop` или папка из `xdg-user-dir DESKTOP`);
- пути подставляются **под твоего пользователя** автоматически (скрипт находит бинарь `ouroboros` через `command -v`, с fallback на `~/.local/bin/ouroboros`) — вручную редактировать `.desktop` не нужно.
